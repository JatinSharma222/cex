...
impl RedisClient {
    ...
    pub fn pop_request(&mut self, queue: &str) -> Option<EngineRequest> {
        ...
        match result {
            Some((_, payload)) => match serde_json::from_str::<EngineRequest>(&payload) {
    ...
    pub fn push_response(&mut self, queue: &str, response: &EngineResponse) {
        let payload = serde_json::to_string(response).expect("Failed to serialize engine response");

        let _: () = self
            .connection
            .lpush(queue, payload)
            .expect("Failed to push response to Redis");
